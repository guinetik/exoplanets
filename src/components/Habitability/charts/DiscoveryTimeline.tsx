  ...
  return (
    <div className="insight-card">
      ...
      <D3Chart aspectRatio={2}>
        {(dimensions) => (
          <TimelineArea data={data} {...dimensions} />
        )}
      </D3Chart>
    </div>
  );
...
function TimelineArea({ data, width, height }: TimelineAreaProps) {
  ...
  useEffect(() => {
    ...
    const x = d3.scaleLinear()
      .domain([d3.min(filteredData, d => d.year) || 1995, d3.max(filteredData, d => d.year) || 2024])
      .range([0, innerWidth]);
    ...
    const areaTotal = d3.area<DiscoveryTrend>()
      .x(d => x(d.year))
      .y0(innerHeight)
      .y1(d => y(d.cumulative))
    ...
    const areaHabitable = d3.area<DiscoveryTrend>()
      .x(d => x(d.year))
      .y0(innerHeight)
    ...
    // Total line
    const lineTotal = d3.line<DiscoveryTrend>()
      .x(d => x(d.year))
      .y(d => y(d.cumulative))
      .curve(d3.curveMonotoneX);
    ...
    // Habitable line
    const lineHabitable = d3.line<DiscoveryTrend>()
      .x(d => x(d.year))
    ...
    const missions = [
      { year: KEPLER_LAUNCH, label: t('pages.habitability.insights.discovery.kepler') },
      { year: TESS_LAUNCH, label: t('pages.habitability.insights.discovery.tess') },
    ];
