...
export function PlanetCard({ planet }: PlanetCardProps) {
  ...
  return (
    <Link to={`/planets/${encodeURIComponent(planet.pl_name)}`} className="planet-card-link">
      <div className="planet-card">
        ...
        <div className="planet-card-flags">
          {planet.is_habitable_zone && (
            <span className="flag-badge flag-habitable">Habitable Zone</span>
          )}
          {planet.is_earth_like && (
            <span className="flag-badge flag-earth-like">Earth-like</span>
          )}
          {planet.cb_flag === 1 && (
            <span className="flag-badge flag-circumbinary">Circumbinary</span>
          )}
        </div>

        {/* Footer hint */}
        <div className="planet-card-footer">
          <span className="planet-card-link-hint">View details →</span>
        </div>
      </div>
    </Link>
  );
